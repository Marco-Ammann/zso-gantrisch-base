...
export class SweepSeqDirective implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    const groups = this.host.nativeElement.querySelectorAll<HTMLElement>('.field-stack--tight');
    groups.forEach(grp => {
      // Wandelt NodeList<Element> in echtes HTMLElement-Array um
      const fields = Array.from(grp.querySelectorAll('zso-input-field')) as HTMLElement[];
