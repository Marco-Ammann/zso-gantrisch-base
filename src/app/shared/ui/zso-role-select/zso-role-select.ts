      ...
      >
        <ng-container *ngIf="selected?.length; else ph">
          <div class="flex flex-wrap gap-1">
            <span
              *ngFor="let r of selected"
              class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-100 dark:bg-primary-900 text-primary-800 dark:text-primary-200"
            >
              {{ r }}
            </span>
          </div>
        ...
        <svg
          class="-mr-1 ml-2 h-4 w-4 text-gray-500 dark:text-gray-400"
...
export class ZsoRoleSelect {
  ...
  isSelected(opt: string) {
    return this.selected?.includes(opt) ?? false;
  }
