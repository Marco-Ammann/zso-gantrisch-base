...
export class Person {
  ...
  get initials(): string {
    return `${this.data.grunddaten.vorname.charAt(
      0
    )}${this.data.grunddaten.nachname.charAt(0)}`;
  }
  ...
  get age(): number | null {
    ...
    const birthDate =
      typeof this.data.grunddaten.geburtsdatum === 'number'
  ...
  get hasDigitalPreference(): boolean {
    return (
      this.data.preferences?.contactMethod === 'digital' ||
  ...
  get hasPaperPreference(): boolean {
    return (
      this.data.preferences?.contactMethod === 'paper' ||
      this.data.preferences?.contactMethod === 'both'
    );
  }
  ...
  get primaryPhone(): string {
    return (
      this.data.kontaktdaten.telefonMobil ||
